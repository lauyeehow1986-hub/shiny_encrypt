...
| **FROST** t-of-n threshold Schnorr signatures (ristretto255) | ✅ working when native backend built (own **Threshold signature (FROST)** tab) |
| **ZK range proof** — prove a hidden value is in [min, max] (Pedersen + bit-OR, ristretto255) | ✅ working when native backend built (own **Zero-knowledge proof (ZK)** tab) |
| **PRE** proxy re-encryption (Umbral) | ✅ working via the optional **GPL** companion `shinyEncryptPRE` |
| **FHE** compute-on-ciphertext — server adds encrypted numbers without the key (TFHE, Zama tfhe-rs) | ✅ working when native backend built (own **Compute on ciphertext (FHE)** tab, size-guarded) |
...
- **Threshold signature (FROST)** — **t-of-n threshold Schnorr signatures** on ristretto255 (reusing the OPRF group), exposed as its own **Threshold signature (FROST)** tab. One signing key is split across **n custodians** so that **any t of them can jointly produce a single ordinary Schnorr signature** and **no custodian ever holds the whole key**. A trusted dealer shares the key with a degree-(t−1) polynomial; signing runs **two rounds** — each chosen custodian first commits to fresh nonces, then (given the whole commitment set) emits a partial signature — and the coordinator sums the partials into one **(R, z)** that verifies like any Schnorr signature under the single group public key. **Binding factors over the entire commitment set** defeat the ROS/Drijvers attack on naive multi-signing, and a **sub-threshold quorum fails closed** (the shares no longer interpolate the group secret, so the signature does not verify). Shipped as a single-machine simulation with an exportable signing transcript. Practical use: require m-of-n sign-off before a dataset release is authorized.

- **Zero-knowledge proof (ZK)** — a **range proof** on ristretto255 (reusing the OPRF group, no new dependency), exposed as its own **Zero-knowledge proof (ZK)** tab. It proves a **hidden value lies in a public range [min, max]** — e.g. that a cohort size is between 100 and 500, or that a column sum falls in a band — **without revealing the value**. The value goes into a **Pedersen commitment** `C = v·G + r·H`; a per-bit **Chaum-Pedersen OR proof** (non-interactive via Fiat-Shamir) shows it decomposes into bits inside the range, and a matching proof on `(max − v)` pins it from above, together proving `min ≤ v ≤ max`. It is **transparent** (no trusted setup) and **publicly verifiable from the proof file alone**; a **false statement cannot be proved** (it fails closed) and a **tampered proof does not verify**. The value can be typed or drawn from a loaded dataset (row count / column sum), in which case it is computed locally and never shown. Practical use: attest that a private statistic is within an agreed bound before releasing it.

- **Compute on ciphertext (FHE)** — **fully homomorphic encryption** (Zama **tfhe-rs**, TFHE, CPU backend), exposed as its own **Compute on ciphertext (FHE)** tab. It demonstrates the defining property of FHE: a **server computes on data it cannot read**. The client encrypts numbers under a secret **client key**; an untrusted party holding **only the evaluation (server) key** homomorphically **adds** the ciphertexts — every add is a real bootstrapped circuit — **without ever decrypting or seeing the values**; the client decrypts the single result and it equals the plaintext sum. The **server key cannot decrypt** (there is no such operation), so nothing about the inputs or the total leaks to the server. **Heavy tier:** keys and ciphertexts are large, so the demo is **size-guarded** (`resource_guard`) and **hard-capped** to a small number of values, and reports the key/ciphertext sizes and a sample ciphertext so the cost is visible. Practical framing: outsource an aggregate over sensitive values to a party you do not trust with the raw data.
